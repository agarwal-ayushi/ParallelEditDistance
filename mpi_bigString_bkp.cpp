...
void distributeData(std::string &X, std::string &Y, char* &pX_char, int* &DP_proc, int &M, int &N, int &pNumCols, int* &pBlockInd, int* &pBlockSize) {
  ...
  //if (pRank == 0) cout << pX_char[61] << pBlockSize[3]<< endl << endl << endl;
  //if (pRank == 0) cout << pX[0] << "\t" << pNumCols << endl;
  //cout << temp[0] << "\t" << pNumCols <<endl;
  // if (pRank == 1) {
...
void createMatrix(std::string X, std::string &Y, char* pX_char, int* &DP_proc, int M, int N, int pNumCols, int* pBlockInd, int* pBlockSize) {
  ...
  for (i=1; i < M+1; i++) {
     ...
     for (j = 1; j < pNumCols; j++) {
       ...
       //if (pRank == 3) cout << i << "\t" << j << "\t" << pX_char[j-1] << "\t" << Y[i-1] << "\t" << DP_proc[i*pNumCols+j] << endl;
     }
     if (pRank < pNum-1) {
...
int main(int argc, char* argv[]) {
  ...
  //if (pRank == 0) cout << pNumCols << "\t" << "\t" << pX_char[0] << "\t"<< pBlockInd[pRank] << "\n\n\n";
  if (pRank == 6) cout << pNumCols << "\t" << "\t" << pX_char[0] << "\t"<< pBlockInd[pRank] << "\n\n\n";
